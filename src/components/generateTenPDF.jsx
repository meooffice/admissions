 ...
 * @param {Object} options - Optional config: { className, registerName, fileName, includeDateOfJoining, includeGender, includeDOB, includeCaste, etc. }
 */
export const generatePDF = (
  ...
  // Title
  const titleText = "Tenth Class Details";
  doc.setFontSize(16);
  ...
  if (includeAdhar) headers.push("Aadhar Number");
  if (includeMoles) headers.push("Identifying Marks");
  if (includeFatherName) headers.push("Father's Name");
  if (includeMotherName) headers.push("Mother's Name");
  if (includeMobileNo) headers.push("Mobile Number");
  if (includeGuardian) headers.push("Guardian");
  ...
  const body = students.map((student) => {
    ...
    if (includeAdhar) row.push(student.st_adhar || "");
    if (includeMoles) row.push(student.moles || "");
    if (includeFatherName) row.push(student.father_name || "");
    if (includeMotherName) row.push(student.mother_name || "");
    if (includeMobileNo) row.push(student.mobile_no || "");
    if (includeGuardian) row.push(student.guardian || "");
  ...
  if (includeAdhar) columnStyles[colIndex++] = { cellWidth: 25 };
  if (includeMoles) columnStyles[colIndex++] = { cellWidth: 35 };
  if (includeFatherName) columnStyles[colIndex++] = { cellWidth: 26 };
  if (includeMotherName) columnStyles[colIndex++] = { cellWidth: 26 };
  if (includeMobileNo) columnStyles[colIndex++] = { cellWidth: 25 };
  if (includeGuardian) columnStyles[colIndex++] = { cellWidth: 26 };
